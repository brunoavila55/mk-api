...
func (s *MKIntegrationService) FetchConexoesByClient(ctx context.Context, sessionToken string, cdCliente string) (map[string]interface{}, error) {
	url := fmt.Sprintf("%s/mk/WSMKConexoesPorCliente.rule?sys=MK0&token=%s&cd_cliente=%s", s.cfg.MKApiURL, sessionToken, cdCliente)
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, url, nil)
	if err != nil {
		return nil, err
	}
	...
	if err := json.Unmarshal(body, &raw); err != nil {
		return nil, fmt.Errorf("failed to parse conexoes response: %w. Body: %s", err, string(body))
	}
...
func main() {
	...
	app.Get("/conexoes", func(c *fiber.Ctx) error {
		key := c.Query("key")
		if cfg.APIKey != "" && key != cfg.APIKey {
			return c.Status(fiber.StatusUnauthorized).JSON(fiber.Map{"error": "Unauthorized API Key"})
		}

		cdCliente := c.Query("cd_cliente")
		if cdCliente == "" {
			return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"error": "Query parameter 'cd_cliente' is required"})
		}
		...
		// 2. Fetch raw conexoes data
		data, err := mkService.FetchConexoesByClient(ctx, sessionToken, cdCliente)
		if err != nil {
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": err.Error()})
		}
